    ...
    public static CardRecognitionData parse(byte[] data) {
        ...
        while (bis.available() >= 2) {
            ...
            if (tlv.getTag().equals(EMVTags.UNIVERSAL_TAG_FOR_OID)) {
                cardRecognitionData.tagAllocationAuthorityOID = Util.decodeOID(tlv.getValueBytes());
                ...
                cardRecognitionData.cardManagementTypeAndVersion = Util.decodeOID(tlv.getValueBytes());
                ...
                tlv = TLVUtil.getNextTLV(new ByteArrayInputStream(tlv.getValueBytes()));
                cardRecognitionData.cardIdentificationScheme = Util.decodeOID(tlv.getValueBytes());
                Log.debug(cardRecognitionData.cardIdentificationScheme);
            }else if(tlv.getTag().equals(GPTags.SECURE_CHANNEL_OID)) {
                tlv = TLVUtil.getNextTLV(new ByteArrayInputStream(tlv.getValueBytes()));
                cardRecognitionData.scpVersionAndOptions = Util.decodeOID(tlv.getValueBytes());
                ...
                if (tlvCcd.getTag().equals(EMVTags.UNIVERSAL_TAG_FOR_OID)) {
                    cardRecognitionData.cardConfigurationDetails = Util.decodeOID(tlvCcd.getValueBytes());
                } else {
                    //Fallback
                ...
                BERTLV tlvCcd = TLVUtil.getNextTLV(new ByteArrayInputStream(tlv.getValueBytes()));
                if (tlvCcd.getTag().equals(EMVTags.UNIVERSAL_TAG_FOR_OID)) {
                    cardRecognitionData.cardChipDetails = Util.decodeOID(tlvCcd.getValueBytes());
                } else {
                    //Fallback
